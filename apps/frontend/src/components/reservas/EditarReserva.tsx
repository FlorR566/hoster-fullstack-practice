...
import {
  ...
  User, Globe, FileText, Mail, Phone,
  Calendar, Clock, Users, BedDouble, Car,
          ...
          <div>
            ...
            <div className="space-y-2">
              {form.serviciosAgregados.map((s, i) => (
                <div key={i} className="bg-[var(--light-main2)] rounded-lg px-4 py-3 flex justify-between items-center">
                  <div>
    ...
    <div className="text-[var(--light-text)] max-w-full">
      ...
      <div className="grid grid-cols-3 gap-x-12 gap-y-2">
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<User size={13} />} label="Nombre completo" value={form.nombreCompleto} />
          <ConfirmRow icon={<Mail size={13} />} label="Email" value={form.email} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="pais-light.svg" darkSvg="pais-dark.svg" />} label="País" value={form.pais} />
          <ConfirmRow icon={<Phone size={13} />} label="Teléfono" value={form.telefono} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="id-light.svg" darkSvg="id-dark.svg" />} label="Tipo de documento" value={form.tipoDocumento} />
          <ConfirmRow icon={<SvgIcon lightSvg="id-light.svg" darkSvg="id-dark.svg" />} label="Número de identidad" value={form.documentoIdentidad} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-x-12 gap-y-2">
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="fecha-llegada-light.svg" darkSvg="fecha-llegada-dark.svg" />} label="Fecha estimada de check-in" value={formatDate(form.fechaCheckin)} />
          <ConfirmRow icon={<SvgIcon lightSvg="hora-llegada-light.svg" darkSvg="hora-llegada-dark.svg" />} label="Hora estimada de llegada" value={form.horaLlegada} />
          <ConfirmRow icon={<Users size={13} />} label="Cantidad de adultos" value={form.adultos.toString().padStart(2, "0")} />
          <ConfirmRow icon={<SvgIcon lightSvg="alojamiento-light.svg" darkSvg="alojamiento-dark.svg" />} label="Tipo de alojamiento" value={form.tipoAlojamiento} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="fecha-salida-light.svg" darkSvg="fecha-salida-dark.svg" />} label="Fecha estimada de check-out" value={formatDate(form.fechaCheckout)} />
          <ConfirmRow icon={<SvgIcon lightSvg="hora-salida-light.svg" darkSvg="hora-salida-dark.svg" />} label="Hora estimada de salida" value={form.horaCheckout} />
          <ConfirmRow icon={<Users size={13} />} label="Cantidad de niños" value={form.ninos.toString().padStart(2, "0")} />
          <ConfirmRow icon={<SvgIcon lightSvg="alojamiento-light.svg" darkSvg="alojamiento-dark.svg" />} label="Alojamiento" value={form.numeroAlojamiento} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="cantidad-noches-light.svg" darkSvg="cantidad-noches-dark.svg" />} label="Cantidad de noches" value={form.cantidadNoches} />
          <div className="invisible pointer-events-none">
            <ConfirmRow icon={<Clock size={13} />} label="placeholder" value="—" />
          </div>
          <ConfirmRow icon={<SvgIcon lightSvg="habitacion-light.svg" darkSvg="habitacion-dark.svg" />} label="Cantidad de habitaciones" value={form.habitaciones.toString().padStart(2, "0")} />
          <ConfirmRow icon={<Car size={13} />} label="Ingresa con vehículo" value={form.ingresaVehiculo} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-x-12 gap-y-4">
        {form.serviciosAgregados.map((s, i) => (
          <div key={i} className="flex flex-col gap-1.5">
            <ConfirmRow icon={<SvgIcon lightSvg="servicios-light.svg" darkSvg="servicios-dark.svg" />} label="" value={s.nombre} />
            <ConfirmRow icon={<Calendar size={13} />} label="Fecha Pedido" value="—" />
            <ConfirmRow icon={<SvgIcon lightSvg="cantidad-noches-light.svg" darkSvg="cantidad-noches-dark.svg" />} label="Cantidad de noches" value="—" />
            <ConfirmRow icon={<DollarSign size={13} />} label="Precio por noche" value={`$${s.precio}`} />
            <ConfirmRow icon={<SvgIcon lightSvg="total-light.svg" darkSvg="total-dark.svg" />} label="Precio total" value={`$${s.precio}`} />
          </div>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-x-12 gap-y-2 mt-3">
        <ConfirmRow icon={<SvgIcon lightSvg="estacionamiento-light.svg" darkSvg="estacionamiento-dark.svg" />} label="Estacionamiento incluido" value={form.estacionamiento} />
        {form.estacionamiento === "Si" && (
          <ConfirmRow icon={<Car size={13} />} label="Patente" value={form.patente} />
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-x-12 gap-y-2">
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<DollarSign size={13} />} label="Precio por noche" value={precioPorNoche > 0 ? `${precioPorNoche} USD` : "—"} />
          <ConfirmRow icon={<DollarSign size={13} />} label="Precio por servicios" value={totalServicios > 0 ? `${totalServicios} USD` : "—"} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="total-light.svg" darkSvg="total-dark.svg" />} label="Precio total por noche" value={totalNoches > 0 ? `${totalNoches} USD` : "—"} />
          <ConfirmRow icon={<SvgIcon lightSvg="total-light.svg" darkSvg="total-dark.svg" />} label="Total estimado de la estadía" value={totalEstadia > 0 ? `${totalEstadia} USD` : "—"} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-x-12 gap-y-2">
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<SvgIcon lightSvg="metodo-pago-light.svg" darkSvg="metodo-pago-dark.svg" />} label="Estado del pago" value={econ.estadoPago} />
          <ConfirmRow icon={<DollarSign size={13} />} label="Monto pendiente" value={saldoPendiente > 0 ? `${saldoPendiente} USD` : "—"} />
          <ConfirmRow icon={<SvgIcon lightSvg="barcode-light.svg" darkSvg="barcode-dark.svg" />} label="Número de recibo/transacción" value={econ.nroRecibo} />
        </div>
        <div className="flex flex-col gap-2">
          <ConfirmRow icon={<CreditCard size={13} />} label="Medio de pago" value={econ.medioPago} />
          <ConfirmRow
